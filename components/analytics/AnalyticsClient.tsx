...
interface CorrelationRow {
  name: string; type: 'food' | 'contact'; count: number
  avgSeverityAfter: number; globalAvg: number; delta: number; suspicious: boolean
}
...
function computeCorrelations(
  ...
  for (const item of items) {
    const name = type === 'food' ? (item as FoodPoint).food_name : (item as ContactPoint).contact_name
  ...
  for (const name of Object.keys(nameMap)) {
    ...
    rows.push({
      name, type, count: occurrences.length,
...
interface TooltipProps { active?: boolean; payload?: { payload: { severity: number; label: string; date: string } }[] }
function CustomTooltip({ active, payload }: TooltipProps) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg p-3 border border-gray-100 text-sm">
      ...
      <p style={{ color: SEVERITY_BG[d.severity] }}>{d.severity} — {d.label}</p>
    </div>
  )
}
...
export default function AnalyticsClient({ skinEntries, foodEntries, contactEntries }: Props) {
  ...
  const cutoff = useMemo(() => new Date(Date.now() - period * 86400000).toISOString().split('T')[0], [period])
  ...
  const chartData = useMemo(() => filteredSkin.map((e) => ({
  ...
  const allCorr     = useMemo(() => [...foodCorr, ...contactCorr].sort((a, b) => b.delta - a.delta).slice(0, 15), [foodCorr, contactCorr])
  ...
  return (
    <div className="max-w-lg mx-auto p-4 space-y-5">
        ...
        <div className="flex gap-1 bg-gray-100 p-1 rounded-xl">
          {([30, 60] as const).map((p) => (
            <button key={p} onClick={() => setPeriod(p)}
              className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${period === p ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-500'}`}>
              {p} дней
            </button>
          ))}
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="card text-center py-4"><p className="text-2xl font-bold text-skin-500">{filteredSkin.length}</p><p className="text-xs text-gray-500 mt-0.5">записей кожи</p></div>
        <div className="card text-center py-4"><p className="text-2xl font-bold text-gray-700">{globalAvg}</p><p className="text-xs text-gray-500 mt-0.5">средний балл</p></div>
        <div className="card text-center py-4"><p className="text-2xl font-bold text-red-500">{suspicious.length}</p><p className="text-xs text-gray-500 mt-0.5">подозрит.</p></div>
      </div>
      <div className="card">
        <p className="text-sm font-semibold text-gray-700 mb-3">📈 Динамика кожи</p>
        {chartData.length === 0 ? <p className="text-center text-gray-400 py-8 text-sm">Недостаточно данных</p> : (
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={chartData} margin={{ top: 5, right: 10, bottom: 5, left: -20 }}>
              ...
              <Line type="monotone" dataKey="severity" stroke="#ff7328" strokeWidth={2.5}
                dot={(props: { cx: number; cy: number; payload: { date: string; severity: number } }) => (
                  <circle key={`dot-${props.payload.date}`} cx={props.cx} cy={props.cy} r={5}
                    fill={SEVERITY_BG[props.payload.severity] ?? '#ff7328'} stroke="white" strokeWidth={2} />
                )} />
            </LineChart>
          </ResponsiveContainer>
        )}
      ...
      {suspicious.length > 0 && (
        <div className="card border-red-100 bg-red-50">
          ...
          <p className="text-xs text-red-600 mb-3">После них в течение 48 ч. кожа в среднем хуже на ≥0.5 балла</p>
          <div className="space-y-2">
            {suspicious.map((r) => (
              <div key={r.name} className="flex items-center gap-3 bg-white rounded-2xl px-4 py-3">
                ...
                <div className="flex-1"><p className="text-sm font-medium text-gray-800">{r.name}</p><p className="text-xs text-gray-400">{r.count}× · ср. балл после: {r.avgSeverityAfter}</p></div>
                <div className="text-right"><p className="text-sm font-bold text-red-500">+{r.delta}</p><p className="text-xs text-gray-400">к норме</p></div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="card">
        ...
        {allCorr.length === 0 ? <p className="text-center text-gray-400 py-4 text-sm">Недостаточно данных для анализа</p> : (
          <div className="space-y-1">
            <div className="grid grid-cols-4 text-xs text-gray-400 font-medium px-2 py-1">
              <span className="col-span-2">Фактор</span><span className="text-center">Ср. балл</span><span className="text-center">Δ к норме</span>
            </div>
            {allCorr.map((r) => (
              <div key={r.name} className={`grid grid-cols-4 items-center px-2 py-2 rounded-xl text-sm ${r.suspicious ? 'bg-red-50' : 'hover:bg-gray-50'}`}>
                ...
                <div className={`text-center font-medium text-sm ${r.delta > 0 ? 'text-red-500' : 'text-green-600'}`}>
                  {r.delta > 0 ? '+' : ''}{r.delta}
                </div>
              </div>
            ))}
        ...
        <div className="card">
          ...
          <div className="flex flex-wrap gap-2">
            {filteredFood.filter((f) => f.is_new_product).map((f) => (
              <span key={f.id} className="badge bg-yellow-100 text-yellow-800 text-xs py-1 px-2">{f.food_name}</span>
            ))}
          </div>
        </div>
      )}
